...
function Items() {
    ...
    // Tối ưu handler functions với useCallback
    ...
    const handleConfirmDelete = useCallback(async () => {
        if (item) {
            try {
                await deleteItem(item.product_code);
                dispatch(fetchItems());
            } catch (error) {
    ...
    // Tối ưu message để tránh render lại không cần thiết
    ...
    // Memoize các TableRow để tránh re-render không cần thiết
    const tableRows = useMemo(() => {
        // Sắp xếp giảm dần theo product_code (số)
        ...
        <>
            ...
            >
                <ConfirmForm
                    ...
                    question="Bạn chắc chắn muốn xoá hàng mặt hàng này chứ ?"
                />
            </Dialog>
            ...
            >
                <ItemsForm
                    onClose={handleCloseForm}
                    type={formType} // 'edit' hoặc 'create'
                    initialValues={item}
                />
            </Dialog>
        </>
