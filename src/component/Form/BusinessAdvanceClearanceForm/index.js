...
import BusinessAdvanceClearanceVouchersForm from '../BusinessAdvanceClearanceVouchersForm';
import BusinessAdvanceClearanceSpendingsForm from '../BusinessAdvanceClearanceSpendingsForm';
import { numberToVietnameseWords, formatNumberWithCommas, parseFormattedNumber } from '../../../utils/numberToWords';
import { useMemo, useEffect } from 'react';

function BusinessAdvanceClearanceForm() {
    ...
    const unspentAmountInWords = () => {
        const amount = requestFormData?.business_advance_clearance_request?.unspent_amount;
            ...
            dispatch(
                setRequestFormData({
                    ...
                    business_advance_clearance_request: {
                        ...requestFormData.business_advance_clearance_request,
    ...
    const totalVoucherAmount = useMemo(() => {
        const vouchers = requestFormData?.business_advance_clearance_request?.vouchers || [];
        return vouchers.reduce((sum, v) => sum + (parseFloat(v.amount) || 0), 0);
    }, [requestFormData]);

    // Tính tổng số tiền đã chi
    const totalSpendingAmount = useMemo(() => {
        const spendings = requestFormData?.business_advance_clearance_request?.spendings || [];
    ...
    const remainAmount = useMemo(() => {
        const unspent = parseFloat(requestFormData?.business_advance_clearance_request?.unspent_amount) || 0;
    ...
    useEffect(() => {
        const advance = requestFormData?.business_advance_clearance_request || {};
        let changed = false;
        ...
        if (changed) {
            dispatch(
                setRequestFormData({
                    ...requestFormData,
                    business_advance_clearance_request: defaultData,
                }),
            );
        }
        ...
        <Stack spacing={3}>
            <Stack direction="row" alignItems="center" spacing={2}>
                ...
                <TextField
                    ...
                    name="address"
                    value={
                        requestFormData?.business_advance_clearance_request?.address !== undefined
                            ? requestFormData.business_advance_clearance_request.address
                            : user?.department || ''
                    }
            ...
            <Stack direction="row" alignItems="center" spacing={2}>
                ...
                <TextField
                    ...
                    name="unspent_amount"
                    value={
                        requestFormData?.business_advance_clearance_request?.unspent_amountText !== undefined
                            ? requestFormData.business_advance_clearance_request.unspent_amountText
                            : '0'
                    }
                    onChange={handleChange}
                    size="medium"
            ...
            <BusinessAdvanceClearanceVouchersForm />
            <Typography sx={{ fontSize: '1.4rem' }}>Số tiền đã chi:</Typography>
            <BusinessAdvanceClearanceSpendingsForm />
                ...
                <>
                    <Stack direction="row" alignItems="center" spacing={2}>
                        ...
                        <Autocomplete
                            ...
                            value={
                                bankOptions.find(
                                    (b) => b.label === requestFormData?.business_advance_clearance_request?.bank_name,
                                ) || null
                            }
                            onChange={(_, newValue) => {
                                dispatch(clearErrors());
                    ...
                    <Stack direction="row" alignItems="center" spacing={2}>
                        ...
                        <TextField
                            ...
                            value={requestFormData?.business_advance_clearance_request?.bank_account_number || ''}
                            onChange={handleChange}
                            size="medium"
                            inputProps={{
                                style: { fontSize: '1.4rem' },
                    ...
                    <Stack direction="row" alignItems="center" spacing={2}>
                        ...
                        <TextField
                            ...
                            value={requestFormData?.business_advance_clearance_request?.beneficiary_name || ''}
                            onChange={handleChange}
                            size="medium"
                            inputProps={{
                                style: { fontSize: '1.4rem' },
...
export default BusinessAdvanceClearanceForm;
