...
function EditToolbar(props) {
    ...
    return (
        <GridToolbarContainer>
            <Tooltip title="Thêm chứng từ mới">
                <Button
                    color="primary"
...
export default function BusinessAdvanceClearanceVouchersForm() {
    ...
    useEffect(() => {
        const vouchersForRedux = rows.map(({ voucher_number, voucher_date, amount }) => ({
            voucher_number,
            voucher_date: voucher_date ? new Date(voucher_date).toISOString().slice(0, 10) : null,
            amount,
        }));
        console.log('Vouchers for Redux:', vouchersForRedux); // Debug: Kiểm tra dữ liệu trước khi dispatch
        dispatch(
            setRequestFormData({
                ...requestFormData,
                business_advance_clearance_request: {
                    ...requestFormData.business_advance_clearance_request,
                    vouchers: vouchersForRedux,
                },
            }),
        );
